...
const createBook = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(req.user._id, { $push: { booksOwned: book._id } });
...
const getBooks = async (req, res) => {
  try {
    ...
    else if (sort === 'price_low') booksQuery = booksQuery.sort({ price: 1 });
    else if (sort === 'price_high') booksQuery = booksQuery.sort({ price: -1 });
...
const getNearbyBooks = async (req, res) => {
  try {
    ...
    for (const book of allBooks) {
      if (book.owner._id.toString() === req.user._id.toString()) continue;
...
const deleteBook = async (req, res) => {
  try {
    ...
    if (!book.isAvailable) {
      return res.status(400).json({ success: false, message: 'Cannot delete a book that is currently rented' });
    }

    await book.deleteOne();
    await User.findByIdAndUpdate(req.user._id, { $pull: { booksOwned: req.params.id } });
...
const addToWishlist = async (req, res) => {
  try {
    ...
    await Book.findByIdAndUpdate(req.params.id, { $addToSet: { usersWaitlisted: req.user._id } });

    res.json({ success: true, message: 'Book added to wishlist', data: user.wishlist });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: error.message || 'Server error' });
  }
...
const removeFromWishlist = async (req, res) => {
  try {
    ...
    await Book.findByIdAndUpdate(req.params.id, { $pull: { usersWaitlisted: req.user._id } });

    res.json({ success: true, message: 'Book removed from wishlist', data: user.wishlist });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: error.message || 'Server error' });
  }
